...
const main = async function () {
	...
	const TOKEN_NAME = hre.ethers.utils.formatBytes32String("ETH DEX Index")
    ...
    if (network === 'hardhat') {
        ...
        const crv: ERC20 = (await hre.ethers.getContractAt(ERC20ABI, TOKENS['CRV'])) as ERC20
		const uni: ERC20 = (await hre.ethers.getContractAt(ERC20ABI, TOKENS['UNI'])) as ERC20
		const sushi: ERC20 = (await hre.ethers.getContractAt(ERC20ABI, TOKENS['SUSHI'])) as ERC20

        console.log(await crv.balanceOf(beakerAddress))
		console.log(await uni.balanceOf(beakerAddress))
		console.log(await sushi.balanceOf(beakerAddress))
    }
}
